...
| 🗜️ | [Compactador](https://github.com/condeDeveloper/compactador) | Java 21 · JUnit 5 · AssertJ | compactador do zero, sem `java.util.zip`: E/S em nível de bit, Huffman canônico transmitido só pelos 256 comprimentos de código — o decodificador remonta a árvore sozinho — com teto de 15 bits para a distribuição de Fibonacci não estourar o inteiro, LZ77 com índice por hash de três bytes e a cópia que se alimenta (mil zeros em menos de 50 bytes). Mede os três métodos e fica com o menor, incluindo não comprimir: no pior caso a saída cresce três bytes, não o dobro |
| 🔤 | [Motor de Regex](https://github.com/condeDeveloper/motor-de-regex) | Java 21 · JUnit 5 · AssertJ | motor de expressões regulares do zero: parser por descida recursiva com a precedência certa, construção de Thompson montando o autômato de baixo para cima e simulação por conjunto de estados, sem retrocesso — o que o torna imune a ReDoS: `(a+)+b` contra 2000 caracteres, que trava `java.util.regex`, roda aqui em milissegundos. Classes de caracteres por faixa, quantificadores com contador `{n,m}`, alternância, âncoras e borda de palavra |
| ⚙️ | [Bytecode](https://github.com/condeDeveloper/bytecode) | Java 21 · JUnit 5 · AssertJ | gerador de arquivos `.class` do zero — pool de constantes, atributo `Code`, descritores — e um compilador de expressões cuja saída a **própria JVM carrega e executa**. Sem ASM. O juiz não é um teste meu: é a JVM, que recusa a classe se a pilha declarada estiver errada, e o `javap` do JDK, que desmonta o arquivo gerado. Pelo caminho aparecem as esquisitices de 1995 — índice do pool começando em 1, `long` e `double` ocupando duas posições, UTF-8 modificado — e o fato de a verificação ser preguiçosa: `defineClass` passa, e o `VerifyError` só vem na ligação. 64 testes |
| 🌲 | [Árvore B](https://github.com/condeDeveloper/arvore-b) | Java 21 · JUnit 5 · AssertJ | árvore B+ em disco, a estrutura por baixo de todo índice de banco relacional. A decisão que a define não é de algoritmo, é de hardware: o nó tem o tamanho de um bloco, então 200 mil chaves cabem em três níveis e qualquer busca custa **três leituras de disco**, contra as vinte de uma árvore binária. Folhas encadeadas fazem uma consulta por faixa custar o tamanho da faixa, não o da tabela. A divisão é por bytes ocupados, não por contagem de chaves. Vinte mil operações sorteadas conferidas contra o `TreeMap`. 24 testes |
| 📡 | [Cliente HTTP](https://github.com/condeDeveloper/cliente-http) | Java 21 · JUnit 5 · AssertJ | cliente HTTP/1.1 sobre `Socket`, sem OkHttp e sem `HttpURLConnection`: corpo em pedaços, gzip, redirecionamento com `Location` relativo, prazos e TLS com verificação de nome — que **não** vem ligada num `SSLSocket`. A armadilha central está na primeira linha do leitor: envolver o soquete num `BufferedReader` engole bytes do corpo que nunca mais voltam. Um teste parte `ação` byte a byte em quatro pedaços. Juízes: o `HttpServer` e o `HttpClient` do próprio JDK. 31 testes |
| 🗂️ | [Ordenação Externa](https://github.com/condeDeveloper/ordenacao-externa) | Java 21 · JUnit 5 | ordenar um arquivo **maior que a memória**: gerar blocos ordenados que caibam, e intercalar k vias com fila de prioridade mantendo só uma linha de cada bloco. A medição contrariou a expectativa — blocos 4× maiores cortaram uma rodada inteira de disco e ficaram **50% mais lentos**, porque a ordenação em memória é n·log n e quadruplicar n custa mais que a passagem economizada. A `PriorityQueue` não é estável, e sem desempate por bloco linhas iguais saem fora de ordem. O juiz é o `List.sort`; os testes forçam o caso difícil pedindo blocos de três linhas. 12 testes |
| 🌿 | [Árvore de Merkle](https://github.com/condeDeveloper/arvore-merkle) | C# 12 · .NET 8 · xUnit | árvore de Merkle e provas de pertencimento, com **raízes de blocos reais do Bitcoin** como juiz — quatro blocos da cadeia principal, incluindo um de nove transações que exercita a duplicação do último nó em três níveis. Provar que um item está numa lista de um milhão custa 20 hashes, 640 bytes. A **CVE-2012-2459** está demonstrada em código: duas listas diferentes com a mesma raiz derrubavam blocos legítimos da rede. E o byte de prefixo do RFC 6962 impede o ataque de segunda pré-imagem que o esquema do Bitcoin permite. 53 testes |
| 🎲 | [Contadores Probabilísticos](https://github.com/condeDeveloper/contadores-probabilisticos) | C# 12 · .NET 8 · xUnit | Bloom, HyperLogLog e Count-Min Sketch, com o erro **previsto por fórmula e medido** contra a resposta exata: 10 milhões de distintos em **16 KB** com 0,06% de erro, contra 572 MB de um `HashSet`. As três taxas do Bloom bateram com as prometidas e deram zero falsos negativos; o Count-Min não subestimou nenhum de 198 mil itens. O `GetHashCode` do .NET não serve — é aleatorizado por processo —, então há um xxHash64 próprio com teste de distribuição bit a bit. 51 testes |
| ⭕ | [Hash Consistente](https://github.com/condeDeveloper/hash-consistente) | C# 12 · .NET 8 · xUnit | o anel que decide em que servidor cada chave mora. Passar de 8 para 9 servidores com `hash % n` remaneja **88,7%** das chaves; o anel remaneja **10,3%**, que é o `1/n` da teoria. As réplicas virtuais não são otimização: com um ponto por servidor, uma máquina ficou com 523 chaves e outra com 76.145. Lista de preferência com servidores distintos, porque sem isso as três réplicas viram a mesma máquina. 30 testes |
| 🧮 | [Decimal](https://github.com/condeDeveloper/decimal) | Java 21 · JUnit 5 | aritmética decimal de precisão arbitrária **sem `BigDecimal` e sem `BigInteger`** — eles aparecem só nos testes, como juiz: **250 mil operações** sorteadas conferidas contra o `BigDecimal` do JDK, nos sete modos de arredondamento. Existe porque binário não representa 0,1: em `double`, `0.1 + 0.2` dá `0.30000000000000004`. Dígitos em base um bilhão, que é potência de dez e deixa deslocar a vírgula sem conta. O arredondamento do banqueiro tem teste provando que ele **não enviesa a soma**, que é o motivo estatístico de existir. 53 testes |
...
| 🚚 | [Precificador de Frete](https://github.com/condeDeveloper/precificador-de-frete) | C# 12 · .NET 8 · ASP.NET Core · xUnit | zona tarifária pelas 31 faixas de CEP dos Correios (com as que se intercalam entre DF e GO e os estados de faixa dupla), peso cubado comparado ao real no total da carga, faixa de peso com excedente por quilo, ad valorem, GRIS, pedágio por fração de 100 kg, área de difícil acesso, ICMS calculado por dentro (`base ÷ (1 − alíquota)`, não `base × 1,12`) e prazo em dias úteis com os feriados móveis saindo do algoritmo de Meeus/Butcher |
| 🧾 | [Simples Nacional](https://github.com/condeDeveloper/simples-nacional) | C# 12 · .NET 8 · ASP.NET Core · xUnit | os cinco anexos da LC 123/2006 com as tabelas se autovalidando na carga do tipo, alíquota efetiva com parcela a deduzir (usar a nominal cobra 11,20% onde o correto é 8,08%), fator R decidindo entre os anexos III e V mais o cálculo de quanto falta de folha para virar, sublimite de R$ 3,6 mi tirando ICMS e ISS do DAS, CPP fora do DAS no Anexo IV, RBT12 anualizado para início de atividade e repartição fechando ao centavo entre União, estado e município |
| 🚦 | [Limitador de Taxa](https://github.com/condeDeveloper/limitador-de-taxa) | C# 12 · .NET 8 · ASP.NET Core · xUnit | cinco algoritmos: janela fixa (com o pico de borda documentado em teste — 100/min deixa passar 200 em um segundo na virada), janela deslizante por registro, janela ponderada que estima a janela móvel com dois contadores, balde de fichas que acumula permissão e aceita rajada, e balde furado que libera em ritmo constante. Relógio injetável para testar sem dormir, contrato comum em classe base, composição em série que pergunta a todos antes de consumir de qualquer um, cota por grupo e middleware com 429, `Retry-After` e cabeçalhos de limite |
...
| | Projeto | Destaques |
|---|---------|-----------|
| 📊 | [Analisador de Logs](https://github.com/condeDeveloper/analisador-de-logs) | analisador de logs em Python puro: leitores de Apache/nginx, JSON por linha, syslog e log de aplicação com detecção automática e suporte a arquivo com formatos misturados, pares `chave=valor` pescados da mensagem, linguagem de filtro com comparação tipada (`nivel >= AVISO` compara por gravidade, não pelo alfabeto; campo ausente segue o critério do SQL para nulo), percentis por interpolação, série temporal com janelas vazias preenchidas, picos e relatório com barras e sparkline. 129 testes, zero dependências |
...
| 🔀 | [Diferença](https://github.com/condeDeveloper/diferenca) | diff unificado do zero pelo algoritmo de Myers, com a saída **idêntica byte a byte à do `git diff`** — mil pares de arquivos sorteados passam pelo git de verdade e pelo código, e as duas saídas têm de bater; o `git apply` ainda aplica os patches gerados. A pergunta ingênua (maior subsequência comum) custa uma tabela de N×M; a de Myers (menor número de edições) custa O((N+M)·D), e em código D é pequeno: 3 mudanças em 10 mil linhas saem em 33 ms. Espaço linear pela busca do ponto do meio a partir das duas pontas. Um teste ia comparar com o `difflib` e falhou — o `SequenceMatcher` não é minimal. 124 testes |
| 🕰️ | [Fuso Horário](https://github.com/condeDeveloper/fuso-horario) | leitor da base de fusos da IANA do zero: o formato binário **TZif** da RFC 8536, a cadeia POSIX TZ e as horas locais que não existem. O juiz é o `zoneinfo` da biblioteca padrão lendo os mesmos arquivos — **40 mil instantes** sorteados entre 1900 e 2050, em oito fusos, zero divergências. O arquivo de Nova York para em **2007**: todo o resto sai de 22 caracteres no fim dele, e quem os ignora deixa janeiro de 2026 em horário de verão. São Paulo mudou de regra 91 vezes desde 1914 e parou em 2019; Lord Howe é o único lugar cujo verão adianta **meia hora**. 38 testes |
...
| 🗃️ | [Arquivador](https://github.com/condeDeveloper/arquivador) | leitor e escritor de **tar** POSIX ustar, e a prova de que está certo é o `tar` do sistema lendo o que ele grava — e ele lendo o que o `tar` grava, nos dois sentidos, na CI. Blocos de 512 bytes, números em octal como texto (daí o limite de 8 GB do formato clássico) e a soma de verificação que cobre a si mesma fingindo que o próprio campo são oito espaços. Extração recusa entrada que escaparia da pasta, inclusive por ligação simbólica. 56 testes |
| 🔑 | [Assinador JWT](https://github.com/condeDeveloper/assinador-jwt) | JWT e JWS sobre `node:crypto`, com os vetores do **apêndice A.1 do RFC 7515** reproduzidos byte a byte. `none` nunca é aceito, e `verificar` exige a lista de algoritmos — porque confiar no `alg` de dentro do token permite trocar o cabeçalho para HS256 e assinar com a chave pública RSA como se fosse o segredo do HMAC. Um teste monta esse ataque inteiro e mostra que não passa. ECDSA em R‖S cru, não em DER. 57 testes |
| 🧭 | [Resolvedor DNS](https://github.com/condeDeveloper/resolvedor-dns) | DNS do zero sobre `node:dgram`, pela RFC 1035. A parte curiosa é a compressão por ponteiro de 1987: um rótulo pode apontar para um deslocamento anterior, o que torna impossível ler um nome sem a mensagem inteira e obriga a recusar ponteiro em laço — senão uma resposta de 20 bytes prende o processo. Como UDP não avisa quando dá errado, há repetição, conferência de ID e pergunta ecoada, e volta para TCP. 64 testes |
| 🔬 | [Analisador JSON](https://github.com/condeDeveloper/analisador-json) | JSON pela RFC 8259, por três coisas que o `JSON.parse` não faz: o erro diz **linha, coluna e o trecho com um cursor embaixo**; chave repetida é erro em vez de ficar com a última em silêncio; e inteiro acima de 2^53 não perde dígito (`9007199254740993` volta inteiro). Mil valores sorteados conferem ida e volta contra o embutido — foi esse teste que pegou o único bug do projeto. 115 testes |
| 📓 | [Diário](https://github.com/condeDeveloper/diario) | log *append-only* com CRC-32, compactação e um chave-valor durável em cima — o desenho do WAL do Postgres e do commit log do Kafka. Como nada é sobrescrito, uma queda estraga no máximo o último registro. O teste que importa não simula: um processo de verdade escreve, grava um pedaço do registro seguinte e **morre**, e a abertura corta o pedaço quebrado. Gravar a mesma chave três vezes dá histórico de graça. 51 testes |
